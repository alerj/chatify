...
function updateSelectedContact(user_id) {
  ...
  $(document)
    .find('.messenger-list-item[data-contact=' + (user_id || getMessengerId()) + ']')
...
let app_modal = function ({
  ...
  // buttons
  buttons == true ? modal.find('.app-modal-footer').show() : modal.find('.app-modal-footer').hide()
    ...
    $.ajax({
      ...
      beforeSend: () => {
        ...
        if (hasFile) {
          messagesContainer
            .find('.messages')
            .append(sendTempMessageCard(inputValue + '\n' + loadingSVG('28px'), tempID))
        } else {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (messagesPage == 1) {
          ...
          const curOffset = lastMsg.offset().top - messagesContainer.scrollTop()
...
channel.bind('messaging', function (data) {
  ...
  playNotificationSound('new_message', !(data.from_id == getMessengerId() && data.to_id == auth_id))
})
...
function setContactsLoading(loading = false) {
  if (!loading) {
    ...
    $('.listOfContacts').append(`<div class="loading-contacts">${listItemLoading(4)}</div>`)
  }
...
function setSearchLoading(loading = false) {
  if (!loading) {
    ...
    $('.search-records').append(`<div class="loading-search">${listItemLoading(4)}</div>`)
  }
...
function deleteConversation(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted) return alert('Error occurred, messages can not be deleted!')
...
function deleteMessage(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted) console.error('Error occurred, message can not be deleted!')
...
$(document).ready(function () {
  ...
  function attachmentValidate(file) {
    ...
    if (!chatify.allAllowedExtensions.includes(fileExtension.toString().toLowerCase())) {
...
function playNotificationSound(soundName, condition = false) {
  if ((document.hidden || condition) && chatify.sounds.enabled) {
    const sound = new Audio(`/${chatify.sounds.public_path}/${chatify.sounds[soundName]}`)
    sound.play()
  }
}
/**
